...
export const CrearAsistencias = () => {
    ...
    useEffect(() => {
        if (idUser) {
            const fetchCourses = async () => {
                try {
                    const response = await axiosInstance.get(`http://localhost:5000/api/CourseInProgress/${idUser}`);
    ...
    const handleSubmit = async () => {
        try {
            ...
            await axiosInstance.post(`http://localhost:5000/api/attendance/professor/${idUser}`, json);
