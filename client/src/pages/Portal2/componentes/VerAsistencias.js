...
const VerAsistencias = () => {
  ...
  const fetchAttendance = async () => {
    try {
      const respuesta = await axiosInstance.get(`http://localhost:5000/api/attendance/professor/all/${idUser}`);
  ...
  const fetchTable = async () => {
    try {
      const answer = await axiosInstance.get(`http://localhost:5000/api/attendanceByDate/${selectedAttendance.idCourseFromCareer_fk}/${selectedAttendance.date}`);
  ...
  const fetchFacecount = async () => {
    try {
      const answer = await axiosInstance.get(`http://localhost:5000/api/attendance/Countface/${selectedAttendance.idCourseFromCareer_fk}/${selectedAttendance.date}`);
      setTable(answer.data);
    } catch (error) {
      console.error("Error al llamar a la API:", error);
    }
  ...
  const fetchCount = async () => {
    try {
      const answer = await axiosInstance.get(`http://localhost:5000/api/attendance/count/${selectedAttendance.idCourseFromCareer_fk}/${selectedAttendance.date}`);
