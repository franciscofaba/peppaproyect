...
function Pagina({ children }) {
  ...
  useEffect(() => {
    ...
    console.log('UserId from cookies:', userId);  // Verificar valor
    ...
    if (userId) {
      ...
      axiosInstance.get(url)
        .then(response => {
          console.log('Full response:', response);  // Verificar la respuesta completa
  ...
  useEffect(() => {
    console.log('Updated state:', idUser, idCareer_fk, userName, email, role);
      ...
      <div className="d-flex d-lg-none align-items-center justify-content-center">
        ...
        {menuOpen && (
          <div className="nav-dropdown-custom" style={{ top: `${menuPosition.top + 5}px`, left: `${menuPosition.left}px` }}>
            <div className="nav-item" onClick={() => window.location.href = "/Portal"}>
            Chatbot
            </div>
            <div className="nav-item" onClick={() => window.location.href = "/Justifications"}>
            {t("prueba.t5")}
            </div>
            <div className="nav-item" onClick={() => window.location.href = "/informacioncursos"}>
            {t("prueba.t6")}
            </div>
          </div>
        )}
