...
function InformacionCursos() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axiosInstance.get(`http://localhost:5000/api/User/${idUser_fk}`);
        ...
        const fetchTable = async () => {
            try {
                const answer = await axiosInstance.get(`http://localhost:5000/api/CourseInProgress/${idUser_fk}`);
                ...
                if (firstIdCareer) {
                    const gradocarreraResponse = await axiosInstance.get(`http://localhost:5000/api/informaciongrados/${firstIdCareer}`);
    ...
    const fetchInformacion = async (idUser_fk) => {
        try {
            const response = await axiosInstance.get(`http://localhost:5000/api/informacioncursos/${idUser_fk}`);
