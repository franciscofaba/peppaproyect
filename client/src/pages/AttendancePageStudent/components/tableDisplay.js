...
const TableDisplay = ({ attendance }) => {
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await axiosInstance.post('http://localhost:5000/api/pdf/upload', formData, {
        headers: {
      ...
      const correo = await axiosInstance.get(`http://localhost:5000/api/enviarEmail/${idUser}`);
    } catch (error) {
