...
export const sendJustificacionEmail = async (req, res) => {
    try {
       const {idUser} = req.params;

       const [rows] = await pool.query('SELECT email FROM Login WHERE idUser_fk = ?', [idUser]);
       console.log(rows);
       ...
       console.log(userEmail);
        ...
        const mailOptions = {
            from: 'bpesociedad@gmail.com',
            to: userEmail, // Cambiar por el correo del destinatario
            subject: 'Estado justificacion',
            text: `Estimado alumno tu justificacion ha sido recibida`
        };
